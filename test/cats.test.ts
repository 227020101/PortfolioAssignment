...
describe('Cat API', () => {
  describe('POST /api/v1/cats', () => {
    it('should create a new cat', async () => {
      const newCat = { name: 'Fluffy', alltext: "outgoing", gender: "male",age: 3,"location": "Kowloon Bay" };
    ...
    it('should return a 400 Bad Request status code when a required field is missing', async () => {
      const newCat = { age: 'Fluffy', breed: 'Persian' };
      const result = await request(app.callback()).post('/api/v1/cats')
        .send(newCat)
        .auth(login, password);
  ...
  describe('Edit /api/v1/users/:id', () => {
    it('should delete an existing user when authenticated', async () => {
      const catData = { name: 'Fluffy', alltext: "outgoing", gender: "male",age: 3,"location": "Kowloon Bay" };
      const result = await request(app.callback()).put('/api/v1/cats/25')
        .send(catData)
        .auth(login, password);
  ...
  describe('DELETE /api/v1/cats/:id', () => {
    ...
    //   const result = await request(app.callback()).delete('/api/v1/cats/66')
    //     .auth(login, password);
